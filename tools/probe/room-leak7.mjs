...
await withGame(async (P) => {
  ...
  const shotB = async (tag, nbN, nbF, iN, iF, db) => {
    const over = `moon.shadow.normalBias=${nbN}; moon.intensity=${iN}; moon.shadow.bias=${db};`
      + ` if(typeof moonFar!=='undefined'&&moonFar){ moonFar.shadow.normalBias=${nbF}; moonFar.intensity=${iF}; moonFar.shadow.bias=${db}; }`
      + ' scene.environment=null;';
    await P('scene.environment=null; 1');
    console.log('  ' + tag.padEnd(30) + await P(HOT(cx, cz, eye, over)));
  };
  ...
  await shot('  sun back on', 0.15, 0.3516, I, IF);

  /* room-leak6: the leaking points sit 1 cm from a concave corner and the wall that should shadow them is
     8 MM away — an order of magnitude under the near cascade's 5.86 cm texel. normalBias cannot reach that
     (measured across its whole range), so the remaining lever three exposes is the DEPTH bias. */
  console.log('\nshadow.bias (depth), stated per row alongside both normalBiases:');
  for(const db of [0, -0.0001, -0.0005, -0.002, -0.008, -0.03]){
    await shotB('shadow.bias ' + db, 0.15, 0.3516, I, IF, db);
  }
  console.log('\nand with normalBias also zeroed, so the two are not fighting:');
  for(const db of [-0.0005, -0.002, -0.008]){
    await shotB('nb 0, shadow.bias ' + db, 0, 0, I, IF, db);
  }
}, { settleMs: 6000 });
