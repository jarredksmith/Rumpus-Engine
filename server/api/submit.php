...
// The alert itself lives in _community_lib.php (build 1346) — report.php is a second caller and
// the config + delivery must exist once. What a submission SAYS is still composed here.
$queueLen = $total + 1;
notifyModerator([
  'subject' => 'level awaiting review — ' . $rec['name'],
  'title'   => 'New level awaiting review',
  'intro'   => 'A new community level was submitted.',
  'line'    => '"' . $rec['name'] . '" by ' . $rec['author']
             . ($rec['desc'] !== '' ? ' — ' . $rec['desc'] : '')
             . ' (' . max(1, round(strlen($rec['code']) / 1024)) . ' KB, ' . $queueLen . ' pending)',
  'cta'     => 'Review + test play',
  'icon'    => '🕹️',
]);
