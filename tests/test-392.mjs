...
assert(/const _jHeld = !!\(keys\[BINDS\.jump\]\|\|padJump\);/.test(src), 'held jump inputs are read (jump bind, build 910)');
...
assert(/if\(!_ledge && _jumpBufT>0 && \(player\.onGround \|\| _coyoteT>0\) && \(player\.jumpCd\|\|0\)<=0[\s\S]*?player\.jumpCd=JUMP_CD;/.test(src), 'jump requires the cooldown to be clear and re-arms it');
