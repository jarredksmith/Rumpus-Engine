...
assert(/if\(heldProp\) txt = \(heldProp\.userData && heldProp\.userData\.grabLabel\) \|\| \(isTouch \? 'Hold USE to drop (?:·|\\u00b7) FIRE to throw' : _pad \? '\[Y\] Drop (?:·|\\u00b7) \[RT\] Throw' : '\[G \/ MMB\] Drop (?:·|\\u00b7) Click Throw (?:·|\\u00b7) Scroll Distance'\);/.test(ug), 'while carrying, a custom label replaces the drop/throw/scroll controls (681; touch 908; pad 934)');
assert(/else if\(_grabAvail\) txt = _grabLabel \|\| \(isTouch \? 'Hold USE to grab' : _pad \? '\[Y\] Grab' : '\[G \/ MMB\] Grab'\);/.test(ug), 'the grab prompt prefers a custom label (pad wording, build 934)');
