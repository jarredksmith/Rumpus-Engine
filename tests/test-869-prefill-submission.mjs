...
assert(/'BREACHLVL:' \+ await encodeLevel\(obj\)/.test(sub), 'prefill uses the share-link codec');
assert(/&level-json=/.test(sub) && /u\.length<=7600/.test(sub), 'prefills the level-json field, capped under GitHub\'s URL budget');
assert(/delete noThumb\.thumb; u=await mkUrl\(noThumb\)/.test(sub), 'retries without the thumbnail before giving up');
