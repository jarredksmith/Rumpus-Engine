...
assert(/if\(!_ledge && _jumpBufT>0 && \(player\.onGround \|\| _coyoteT>0\) && \(player\.jumpCd\|\|0\)<=0 && !_levelLoaderActive && matchWarmup<=0 && !mountedTurret\)\{ player\.vel\.y = JUMP/.test(src), 'jump is blocked during warmup');
