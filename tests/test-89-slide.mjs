...
assert(/let sliding=false, slideT=0, slideCD=0, _prevSlideKey=false, _slideBufT=0, _sprintGraceT=0; const slideDir=new THREE\.Vector3\(\); const SLIDE_DUR=0\.55;/.test(src), 'slide state + duration (926: tap buffer; 927: sprint grace)');
assert(/const _slideEdge = _slideKey && !_prevSlideKey;/.test(src), 'slide key (C) press is edge-detected (tap, not hold) — build 377');
assert(/if\(!sliding && slideCD<=0 && _slideBufT>0 && \(_sprinting \|\| _sprintGraceT>0\) && player\.onGround && wish\.lengthSq\(\)>0\.01/.test(src), 'only slides while running on the ground (926: buffered tap; 927: sprint grace)');
