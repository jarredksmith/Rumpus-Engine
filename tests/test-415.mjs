...
// build 1355: 1226's `!en.friendly` became `!passive` — a pacifist OR an ally with nothing to fight both
// skip the raycast, and the shared budget is what they are skipping for. Throttle + budget intent unchanged.
assert(/if\(!passive && \(en\._losT==null \|\| now - en\._losT > \(en\._losIv\|\|110\)\) && \(typeof _losBudget==='undefined' \|\| _losBudget>0\)\)\{/.test(src), 'enemy LOS (segmentBlocked) is throttled ~9x/sec per enemy AND capped per frame by _losBudget (build 547)');
