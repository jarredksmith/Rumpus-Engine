...
assert(/playerModelCfg/.test(bav) && /_loadGLTFWithLib\(mc\.url, mc/.test(bav), 'avatar loads the player model when a url is set (via the anim-library shim, build 919)');
...
assert(/const PLAYER_MODEL0 = \{ url:'', scale:1, face:0, rx:0, rz:0, yoff:0, xoff:0, zoff:0, thumb:'', animLib:'', ikHold:true, jointFix:\{\}, autoRig:null, clips:\{ idle:'', walk:'', run:'', attack:'' \} \}/.test(src), 'player model config default (build 919 animLib; build 937 ikHold)');
...
assert(/player:  \{ url: playerModelCfg\.url, thumb: playerModelCfg\.thumb\|\|'', animLib: playerModelCfg\.animLib\|\|'', autoRig: playerModelCfg\.autoRig\|\|undefined, ikHold:\(playerModelCfg\.ikHold===false\)\?false:true, jointFix:_sanitizeJointFix\(playerModelCfg\.jointFix\), state: Object\.assign\(\{\}, editorTargets\.player\.state\), clips: Object\.assign\(\{\}, playerModelCfg\.clips\), clipSpeed: Object\.assign\(\{\}, playerModelCfg\.clipSpeed\|\|\{\}\), clipHold: Object\.assign\(\{\}, playerModelCfg\.clipHold\|\|\{\}\), clipInPlace: Object\.assign\(\{\}, playerModelCfg\.clipInPlace\|\|\{\}\), grip: _sanitizeGripMap\(tpGunGrips\), view: _snapshotView\(\) \}/.test(src), 'serializeLevel writes the player avatar (with speed+hold+in-place, build 493; animLib 919; ikHold 937)');
