...
// ---------------------------------------------------------------- the wiring
{
  assert(/tone\(\{freq:L\.sub\[0\], type:'sine', dur:L\.sub\[1\], vol:L\.sub\[2\], slideTo:Math\.max\(20, L\.sub\[0\]\*0\.55\), attack:0\.002\}\);/.test(src),
    'shoot() fires the sub layer with a fast attack (a thump, not a hum)');
  assert(/tone\(L\.body\); noise\(L\.crack\);/.test(src), '...then the tuned body+crack pair');
  assert(/setTimeout\(\(\)=>noise\(\{dur:L\.tail\[0\], vol:L\.tail\[1\], filterFreq:L\.tail\[2\], type:'lowpass'\}\), L\.tail\[3\]\);/.test(src),
    '...then the delayed tail');
