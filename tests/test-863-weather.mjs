...
assert(/_weatherPts\.position\.set\(cx, cy-6, cz\)/.test(uw), 'the box follows the camera');
assert(/y=_WEATHER_BOX\.h\*\(0\.8\+Math\.random\(\)\*0\.2\)/.test(uw), 'fallen drops respawn at the top (recycled, not reallocated)');
assert(/Math\.sin\(_weatherT\*1\.3\+d\[i\*2\+1\]\)/.test(uw), 'snow sways per-drop');
assert(/a\[i\*3\]\+=1\.6\*dt/.test(uw), 'rain leans with the wind');
