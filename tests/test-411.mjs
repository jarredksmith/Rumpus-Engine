...
assert(/function _emptySounds\(\)\{ return \{ shoot:\{\}, reload:'', explode:'', coin:'', hit:'', kill:'', hurt:'', turret:'' \}; \}/.test(src), 'turret slot exists in the sounds shape');
assert(/'reload','explode','coin','hit','kill','hurt','turret'\]\)\{ if\(typeof s\[k\]==='string'\)/.test(src), 'sanitizeSounds keeps the turret URL');
assert(/'reload','explode','coin','hit','kill','hurt','turret'\]\) loadSound/.test(src), 'preloadCustomSounds preloads the turret sound');
