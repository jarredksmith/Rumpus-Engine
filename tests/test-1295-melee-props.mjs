...
// ---------------------------------------------------------------- 3. a client can swing too
{
  ...
  // build 1443 appended `true` — the swing declares itself an AIMED hit, so it shows a damage number.
  // What this asserts is unchanged: the host applies the damage directly and shoves what survived.
  assert(/else \{ const broke=damageProp\(o, DMG, pt, dir, 8, NET\.myId,? ?\w*\); if\(!broke\) pushDynamic\(o, dir, 8, pt\); \}/.test(block),
