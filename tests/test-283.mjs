...
// build 1355: 1226's `!en.friendly` became the named `passive` predicate (a pacifist OR an ally with
// nothing left to fight this frame). The range+sight intent this pin exists for is untouched.
assert(/if\(dist <= detect && sees && !passive\)\{ en\.aware = true;/.test(fn), 'patrol/hold engage requires range AND sight');
assert(/const passive = en\.friendly \|\| en\._noTgt;/.test(fn), '...and "not fighting anyone" is one predicate, not two conditions');
