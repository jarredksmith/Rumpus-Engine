...
assert(/if\(!_fr && run\.lifesteal>0\) player\.hp = Math\.min\(player\.maxHp, player\.hp \+ run\.lifesteal\)/.test(src), 'lifesteal hook in killEnemy');   // build 1226: gated off friendly deaths — a hostile kill still heals
assert(/else if\(NET\.mode==='off' && gameCfg\.upgrades !== false\)\{ beginUpgradeChoice\(\); \}/.test(src), 'solo + enabled wave-clear opens the picker');
assert(/game:\s*\{ mode: gameCfg\.mode, pvp: gameCfg\.pvp\|\|undefined, pvpTarget: gameCfg\.pvpTarget\|\|undefined, winWaves: gameCfg\.winWaves, upgrades: gameCfg\.upgrades !== false, objective: gameCfg\.objective, surviveSecs: gameCfg\.surviveSecs, raceLaps: gameCfg\.raceLaps, raceBots: gameCfg\.raceBots, raceBotSkill: gameCfg\.raceBotSkill, extractHold: gameCfg\.extractHold, extractRadius: gameCfg\.extractRadius, bossWave: gameCfg\.bossWave, wavesText: \(gameCfg\.wavesText\|\|''\)\.slice\(0,2000\) \|\| undefined, enemyMods: _sanitizeEnemyMods\(gameCfg\.enemyMods\) \|\| undefined, noRespawn: !!gameCfg\.noRespawn, unarmed: !!gameCfg\.unarmed, startWeapon: \(WEAPONS\[gameCfg\.startWeapon\] && !WEAPONS\[gameCfg\.startWeapon\]\.melee\) \? gameCfg\.startWeapon : 'rifle', allowPickup: gameCfg\.allowPickup!==false, flashlight: !!gameCfg\.flashlight, flash: \{ intensity:\+gameCfg\.flashCfg\.intensity, angle:\+gameCfg\.flashCfg\.angle, penumbra:\+gameCfg\.flashCfg\.penumbra, distance:\+gameCfg\.flashCfg\.distance, color:gameCfg\.flashCfg\.color \}, goalText: \(gameCfg\.goalText\|\|''\)\.slice\(0,160\), respawnOnDeath: !!gameCfg\.respawnOnDeath, winText: \(gameCfg\.winText\|\|''\)\.slice\(0,240\), loseText: \(gameCfg\.loseText\|\|''\)\.slice\(0,240\), spawnRegion:/.test(src), 'upgrade toggle saved with the level');
