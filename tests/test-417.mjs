...
assert(/n:NET\.name\|\|'Host', mt:\(mountedTurret\?_turretIndexOf\(mountedTurret\):-1\), cl:_climbCode\(\), c:_packCar\(\) \}\];/.test(src), 'host snapshot self-entry carries its mounted-turret index');
...
assert(/n:NET\.name, mt:\(mountedTurret\?_turretIndexOf\(mountedTurret\):-1\), cl:_climbCode\(\), c:_packCar\(\) \}\); \}catch/.test(src), 'client state packet carries its mounted-turret index');
