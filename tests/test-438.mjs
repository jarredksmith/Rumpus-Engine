...
assert(/const _attBase = \{\}/.test(src) && /_attBase\[k\]=\{ magSize:WEAPONS\[k\]\.magSize, spread:WEAPONS\[k\]\.spread \}/.test(src), 'base stats snapshotted once so re-apply is idempotent');
...
assert(/adsFovLive = a\.fov \* \(typeof _attZoomMul==='function'\?_attZoomMul\(key\):1\)/.test(src), 'scope zoom feeds the ADS fov');
