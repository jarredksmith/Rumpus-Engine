...
assert(/sfxBus\.connect\(_cmp\); _cmp\.connect\(masterBus\)/.test(src) && /catch\(e\)\{ sfxBus\.connect\(masterBus\); \}/.test(src) && /musicBus=actx\.createGain\(\)/.test(src) && /musicBus\.connect\(_musicDuck\); _musicDuck\.connect\(masterBus\)/.test(src), 'sfx feeds master through the 1211 bus compressor (plain connect as the fallback); music feeds master through the 1374 sidechain duck');
