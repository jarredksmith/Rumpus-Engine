...
assert(/addEventListener\('mousedown', e=>\{\n  if\(radialOpen\)\{ if\(e\.button===0\) closeRadial\(true\); return; \}\n  if\(typeof buildMode!=='undefined' && buildMode\)\{[\s\S]{0,340}?\n  if\(shopOpen \|\| editorOpen \|\| paused \|\| mapOpen \|\| duelDead \|\| invOpen\b[^)]*\) return;/.test(src), 'mousedown ignores fire while paused (build 928: the build-mode branch sits between)');
