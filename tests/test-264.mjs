...
/* build 1499: the per-type audio call became the table-derived zone sweep (_edZoneMarkersVisible), which
   hides audio rings and every other zone type at once — same intent, one derivation. */
assert(/if\(typeof setSpawnMarkersVisible==='function'\) setSpawnMarkersVisible\(false\);[\s\S]{0,400}if\(typeof _edZoneMarkersVisible==='function'\) _edZoneMarkersVisible\(false\);/.test(src), 'spawn + pickup + zone markers hidden at the flip');
...
assert(/if\(side \|\| height \|\| _cc \|\| \(typeof tpTilt==='number' && tpTilt\)\)\{ _tpLookAt\.set\(camx \+ fx, camy \+ fy, camz \+ fz\); camera\.lookAt\(_tpLookAt\); \}/.test(tp), 'view stays parallel to forward so the crosshair is accurate');
